<template>
    <div class="detaillist-box">
        <header>
            <van-nav-bar :title="detailType" left-arrow @click-left="router.go(-1)">
                <template #right>
        ...
        <div class="rem" @scroll="scrollEvt">
...
import { listTypeApi, recommendDetailTypeApi } from '../../apis/home.js'
...
function getList() {
    setTimeout(() => {
        recommendDetailTypeApi(page.value, 6, fromHomeParam.value,detailType.value).then(_res => {
            if (_res.code === 200) {
...
watch(
    ...
    function () {
        ...
        detailType.value=route.query.detailType
        listTypeApi().then(d => {
            if (d.code === 200) {
...
.detaillist-box {
    ...
    header {
        ...
        .van-nav-bar {
            height: 44px;
            background-image: url(../../image/category_top_bg.fb3ad63d.png);
