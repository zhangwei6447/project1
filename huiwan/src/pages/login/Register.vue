    ...
    <div class="reg-box">
        ...
        <div class="content" id="test">
            ...
            <van-button
                ...
                type="primary"
                block
                color="linear-gradient(97.04deg,#e35e86,#fb619c 54.05%,#ff419d)"
                id="butn"
                >注册</van-button
            >
        </div>
...
function regEvt() {
    ...
    if (
        user.value.password &&
        re_password.value &&
        user.value.password !== re_password.value
    ) {
        showToast({
            message: "两次输入的密码不一致",
            position: "top",
    ...
    node.value
        ...
        .then(async () => {
            ...
            if (res.code === 200) {
                ...
                if (
                    res.message ===
                    "用戶編碼【" + user.value.username + "】已存在，請重新錄入"
                ) {
                    showToast({
                    ...
                    res.message ===
                    "電話號碼【" + user.value.phone + "】已被注冊，請重新錄入"
                ) {
                    showToast({
                        message: "该手机号已被注册",
                        position: "top",
